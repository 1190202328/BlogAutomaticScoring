...
from src.EDU.dataset import EDUDataset, TransDataset, RlatDataset, AugDataset
            ...
            if len(sent) >= 3:
                for idx1 in range(len(sent) - 1):
                    for idx2 in range(idx1 + 1, len(sent)):
                        ...
                        traindataRlat.append(
                            [sent[idx1], sent[idx2], newRelation + "_" + str(info.iloc[idx, 4]), info.iloc[idx, 2]])
                        self.numData += 1
            else:
                newRelation = self.changeRelationCdtb(info.iloc[idx, 2])
                ...
                if traindataRlat[self.numData - 1][2].split("_")[0] != "coordination" and oversample:
                    ...
                    traindataRlat.append(
                        [traindataRlat[self.numData - 1][1], traindataRlat[self.numData - 1][0], newLabel])
                    self.numData += 1
